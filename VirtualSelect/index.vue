<template>
  ...
  >
    ...
    <span :class="{flex:!!$slots.addition}">
      <el-input
      ref="reference"
      :style="{zIndex: needMask && visible && emptyText !== false ? 801 : ''}"
      ...
      :class="[{ 'is-focus': visible} ,{'flag-1':true}]"
    ...
    <span @click.stop="visible = false" v-if="$slots.addition" :style="{zIndex: needMask && visible && emptyText !== false ? 801 : ''}">
      <slot name="addition"></slot>
    </span>
    </span>
    <transition name="fade">
      <div v-show="needMask && visible && emptyText !== false" class="mask"></div>
    </transition>
    <transition
      name="el-zoom-in-top"
      ...
      <select-menu
        ...
        >
          ...
          ref="virtualList"
            ...
            @tobottom="virtualListEvents('tobottom')"
            @totop="virtualListEvents('totop')"
            v-show="
              virtualListProps &&
              virtualListProps.dataSources &&
              virtualListProps.dataSources.length > 0 &&
              !loading
            "
          >
            <el-option
...
export default {
  ...
  computed: {
    ...
    // virtualListProps() {
    //   return {
    //     dataSources: this.tempDataSources,
    ...
    // },
    // tempDataSources() {
    //   return this.options || this.dataSources
    // }
  },
  components: {
    SelectMenu,
  ...
  methods: {
    ...
    scrollToOption(option) {
      ...
      target && this.$refs.virtualList &&
        this.$refs.virtualList.scrollToOffset(target.offsetTop)
    },
    handleMenuEnter() {
  ...
  created() {
    ...
    this.debouncedOnInputChange = debounce(this.debounce, (v) => {
